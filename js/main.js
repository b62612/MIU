...
localStorage.key(0);
			...
			urgencyValue ;					
					var item 			= {};
						item.agegrades	= ["Age Category:", $("agegrades").value];
			...
			function getData(){
					switchControls("on");
							...
							alert("Local Storage is empty. Default data is loaded");
							autoLoadData();
							}
				...
				getThumbnail(dataObj.agegrades[1], makeSubList);
			...
			function getThumbnail(conCatImg, makeSubList){
					var thumbList = document.createElement("li");
					makeSubList.appendChild(thumbList);
					var agegradeImg = document.createElement("img");
					var setSrc = agegradeImg.setAttribute("src", "images/" + conCatImg + ".png");
					thumbList.appendChild(agegradeImg);
					}
			...
			switchControls("off");
						...
						$("agegrades").value = item.agegrades[1];
				...
				for(var i =0; i<radios.length; i++){
					...
					if(item.urgency[1] == "Yes"){$("urgency").setAttribute("checked", "checked")
					}
			...
			submitData(this.key);
					...
					var ageGrades = ["-- Age Category --", "U_17", "U_23", "Over_23"],
